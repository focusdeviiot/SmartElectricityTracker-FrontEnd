...
import { getReportDeviceWatt } from "../../api/api";
...
const WattLineChart: React.FC<WattLineChartProps> = ({
  ...
  const [wattUsage, setWattUsage] = useState<DatasetsProps>({ x: [], y: [] });
  ...
  const onSearch = async () => {
    ...
    try {
      const response = await getReportDeviceWatt(reqData);
      if (response.success) {
        setWattUsage({ x: response.data.created_at, y: response.data.watt });
      }
    } catch (error: any) {
      ...
      <form onSubmit={handleSubmit(onSearch)} onReset={onReset}>
        <div className="flex flex-wrap justify-center items-end m-4 gap-6">
          ...
          <div className="flex gap-3">
            ...
            <ExportToExcel
              disabled={wattUsage && wattUsage.x.length === 0}
              data={wattUsage}
              fileName={"Watt"}
            />
          </div>
